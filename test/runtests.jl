...
@testset "PythonRNGs" begin
    ...
    @testset "NumPyRandom matches numpy.random.default_rng($seed)" for seed in (0, 1234)
    ...
    @testset "seed! reproduces the stream" for rng in (PythonRandom(42), NumPyRandom(42))
        a = rand(rng, 10)
        Random.seed!(rng, 42)
        @test rand(rng, 10) == a
    end
    ...
    @testset "uniform scalar types" for rng in (PythonRandom(7), NumPyRandom(7))
    ...
    @testset "ranges and arrays" for rng in (PythonRandom(99), NumPyRandom(99))
    ...
    @testset "integer draws are reproducible" for rng in (PythonRandom(3), NumPyRandom(3))
